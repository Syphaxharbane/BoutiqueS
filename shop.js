...
async function loadHomeProducts(){
 ...
 if(!g)return;
 g.innerHTML='<div class="empty">Chargement des produits...</div>';
 if(typeof supabaseClient==='undefined'){
   g.innerHTML='<div class="empty">Erreur : Supabase n’est pas connecté. Vérifiez config.js.</div>';
   return;
 }
 ...
 if(error){
   ...
   g.innerHTML='<div class="empty">Impossible de charger les produits.<br><small>'+escapeHtml(error.message||'Erreur Supabase')+'</small></div>';
   return;
 }
 if(!data?.length){g.innerHTML='<div class="empty">Aucun produit pour le moment.</div>';return}g.innerHTML=data.map((p,i)=>`<article class="product-card" style="animation-delay:${i*60}ms"><a href="product.html?id=${p.id}"><div class="product-image">${p.image_url?`<img src="${escapeHtml(p.image_url)}" alt="">`:''}</div><div class="product-info"><h3>${escapeHtml(p.name)}</h3><div class="price">${money(p.price)}</div><div class="muted">${escapeHtml((p.description||'').slice(0,70))}</div></div></a><div class="product-info"><button class="btn btn-primary" style="width:100%" onclick='addToCart(${JSON.stringify({id:p.id,name:p.name,price:p.price,image_url:p.image_url||''})})'>Ajouter au panier</button></div></article>`).join('');updateCartBadge()}
...
async function initCheckout(){
 updateCartBadge();
 const c=getCart(),s=document.getElementById('checkoutSummary'),w=document.getElementById('wilaya'),co=document.getElementById('commune');
 ...
 try{
   const response=await fetch('https://raw.githubusercontent.com/othmanus/algeria-cities/master/json/algeria_cities.json',{cache:'force-cache'});
   if(!response.ok) throw new Error('Impossible de charger les communes');
   const cities=await response.json();
   cities.forEach(city=>{
     const code=String(city.wilaya_code).padStart(2,'0');
     if(!communesByWilaya[code]) communesByWilaya[code]=[];
     if(city.commune_name_ascii && !communesByWilaya[code].includes(city.commune_name_ascii)) communesByWilaya[code].push(city.commune_name_ascii);
   });
   Object.values(communesByWilaya).forEach(list=>list.sort((a,b)=>a.localeCompare(b,'fr')));
 }catch(err){
   console.error(err);
   s.innerHTML='<div class="card message error">Impossible de charger les communes. Vérifiez votre connexion Internet puis rechargez la page.</div>';
 }

 w.innerHTML='<option value="">Choisir une wilaya</option>'+WILAYAS.map(x=>`<option value="${x[0]}">${x[0]} - ${x[1]} — ${money(x[2])}</option>`).join('');
 ...
 function loadCommunes(){
   ...
   co.innerHTML='<option value="">Choisir une commune</option>'+list.map(x=>`<option value="${escapeHtml(x)}">${escapeHtml(x)}</option>`).join('');
 }
 function summary(){
 ...
 form.onsubmit=async e=>{
   ...
   if(error){m.className='message error';m.textContent=error.message;return}
   localStorage.removeItem('cart');
   m.className='message ok';
   m.innerHTML=`Commande enregistrée ! Numéro : <b>${data.id}</b><br>Gardez ce numéro pour suivre votre commande.`;
   form.reset();
   co.innerHTML='<option value="">Choisir d’abord une wilaya</option>';
   summary();
   updateCartBadge();
...
updateCartBadge();if(document.getElementById('productsGrid'))loadHomeProducts();
